...
# Defaults
#
# Different CSV files use different fields. These are the defaults
# from my financial institution, and can be specified via arguments.

%Options =
  (

   # RecordRE is used to match lines to import, and exclude any
   # comments or junk, Mine all start with the date

   'RecordRE' => "^[0-9][0-9]\/[0-9][0-9]\/",

   # CSV field layout, a comma delimited list of fields being read
   # from each line, This is used to match position to label
   ...
   'FileMatchFile' => -e "./.FileMatches.yaml" ? "./.FileMatches.yaml" : undef,

   'AccountMatchFile' => -e "./.AccountMatches.yaml" ? "./.AccountMatches.yaml" : undef,

   'PreProcessFile' => -e "./.Preprocess.yaml" ? "./.Preprocess.yaml" : undef,
